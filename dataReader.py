...
types = {
    ...
    "Provincia": str,
    "Distrito": str,
    "Número de estacionamiento": str,
    "Depósitos": np.float64,
    "Latitud (Decimal)": np.float64,
    "Longitud (Decimal)": np.float64,
    "Categoría del bien": str,
    "Posición": str,
    "Número de frentes": np.float64,
    "Edad": np.float64,
    "Elevador": np.float64,
    "Estado de conservación": str,
    "Método Representado": str,
    "Área Terreno": str,
    "Área Construcción": str,
...
class dataReader:
    def _read_xlsx(self, file):
        ...
        try:
            ...
            df["Área Terreno"] = df["Área Terreno"].apply(
                lambda x: float(str(x).replace(",", ""))
            )
            df["Área Construcción"] = df["Área Construcción"].apply(
                lambda x: float(str(x).replace(",", ""))
            )
    ...
    def read_data(self, file):
        if file.endswith(".xlsx") or file.endswith(".xls"):
            ...
            log.error(
                "Invalid format. Unrecognized file format. Make sure file ends with .xlsx | .xls | .tsv | .csv"
            )
            sys.exit(1)
        return G
